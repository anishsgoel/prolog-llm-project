...
Constraints:
- Output ONLY productof/2 FACTS. No rules.
- Each fact MUST end with a period.
- Prefer using only constants and predicates that appear in the snapshot or the GOAL.
- Try to include only direct connections, if a suggested edge might be a "shortcut" (skips intermediate subgoals in the proof), give it low confidence.
- You can return empty list, if no atoms are suitable.
...
def main() -> None:
    ...
    kb_missing_obj = kb_obj.omit_facts({4, 8})
